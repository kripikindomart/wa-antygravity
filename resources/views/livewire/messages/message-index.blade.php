<div class="space-y-6 animate-fade-in-up">
    ...
    <div class="flex p-1 space-x-1 bg-slate-100 dark:bg-slate-800 rounded-xl mb-6">
        <button wire:click="$set('activeTab', 'sent')"
            class="w-full py-2.5 text-sm font-medium leading-5 rounded-lg ring-offset-2 focus:outline-none focus:ring-2 {{ $activeTab === 'sent' ? 'bg-white dark:bg-slate-700 shadow text-emerald-700 dark:text-emerald-400 ring-offset-emerald-400 ring-white ring-opacity-60' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">
            Sent Messages
        </button>
        <button wire:click="$set('activeTab', 'inbox')"
            class="w-full py-2.5 text-sm font-medium leading-5 rounded-lg ring-offset-2 focus:outline-none focus:ring-2 {{ $activeTab === 'inbox' ? 'bg-white dark:bg-slate-700 shadow text-emerald-700 dark:text-emerald-400 ring-offset-emerald-400 ring-white ring-opacity-60' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">
            Inbox (Incoming)
        </button>
    </div>

    <!-- Message Composer (Only for Sent tab) -->
    @if($activeTab === 'sent')
    ...
    <div class="glass-card rounded-2xl p-6">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold text-slate-800 dark:text-white">
                {{ $activeTab === 'inbox' ? 'Inbox (Incoming Messages)' : 'Sent History' }}
            </h3>
            @if($activeTab === 'sent')
        ...
        <div class="space-y-3">
            @forelse($messages as $msg)
                <div
                    ...
                    @php
                        $displayNumber = $activeTab === 'inbox' ? $msg->from_number : $msg->to;
                        $displayName = $activeTab === 'inbox' ? ($msg->from_name ?: $msg->from_number) : $displayNumber;
                        $initials = strtoupper(substr(preg_replace('/[^a-zA-Z]/', '', $displayName) ?: 'U', 0, 2));
                        if (empty($initials))
                            $initials = 'U';
                    @endphp

                    <div
                        class="w-10 h-10 rounded-full {{ $activeTab === 'inbox' ? 'bg-gradient-to-br from-blue-400 to-indigo-500' : 'bg-gradient-to-br from-emerald-400 to-teal-500' }} flex items-center justify-center text-white font-semibold text-sm shadow-lg flex-shrink-0">
                        {{ $initials }}
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                            <p class="text-sm font-medium text-slate-800 dark:text-white">
                                {{ $displayName }}
                                @if($activeTab === 'inbox' && $msg->from_name)
                                    <span class="text-xs font-normal text-slate-400">({{ $msg->from_number }})</span>
                                @endif
                            </p>
                            @if($msg->device)
                    ...
                    <div class="text-right flex-shrink-0">
                        @if($activeTab === 'sent')
                            @if($msg->status === 'sent')
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">Sent</span>
                            ...
                            @elseif($msg->status === 'failed')
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400">Failed</span>
                            @endif
                        @else
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">Received</span>
                        @endif

                        <p class="text-xs text-slate-400 mt-1">
                            {{ $activeTab === 'inbox' ? $msg->received_at->diffForHumans() : $msg->created_at->diffForHumans() }}
                        </p>
                    </div>
            ...
            @empty
                <div class="text-center py-8">
                    ...
                    <p class="text-slate-500 dark:text-slate-400">No {{ $activeTab === 'inbox' ? 'incoming' : 'sent' }}
                        messages found.</p>
                </div>
            @endforelse
        </div>
